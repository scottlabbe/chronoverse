...
export default function SharePoemDialog({
  ...
  const handleDownload = useCallback(() => {
    ...
    link.download = `present-verse-${tone.toLowerCase()}-${Date.now()}.png`;
  ...
  const handleNativeShare = useCallback(async () => {
    ...
    try {
      const file = new File([capture.blob], 'present-verse-poem.png', { type: 'image/png' });
      if (navigator.canShare && !navigator.canShare({ files: [file] })) {
        throw new Error('unsupported');
      }
      await navigator.share({
        files: [file],
        text: `A ${tone.toLowerCase()} time poem from The Present Verse.`,
        title: 'The Present Verse poem',
      });
    ...
    if (styleVariant === 'polaroid') {
      ...
      return (
        ...
        >
          ...
          >
            <div style={{ letterSpacing: '0.35em', fontSize: '14px', color: '#e9e9e9' }}>The Present Verse</div>
        ...
        <div className="space-y-4">
          ...
          {capture && !error && (
            <div className="border rounded-lg overflow-hidden">
              <img
                src={capture.dataUrl}
                alt={`The Present Verse ${tone} poem`}
