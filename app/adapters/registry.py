    ...
    def _scan_item(item, allow_reasoning=False):
                ...
                for block in content:
                    ...
                    if block_type in ("output_text", "text", "message"):
                        if isinstance(block, dict):
                            s = (
                                _val_dict(block, "value")
                                or _val_dict(block, "text")
                                or block.get("value")
                                or block.get("text")
                            )
                            if isinstance(s, str) and s.strip():
        ...
        if summary is not None:
            ...
            if isinstance(summary, dict):
                s = (
                    _val_dict(summary, "output_text")
                    or _val_dict(summary, "text")
                ...
                if hasattr(item, "model_dump"):
                    ...
                    if "summary" in d:
                        ...
                        if isinstance(sm, dict):
                            ...
                            sv = (
                                sm.get("text")
                                or sm.get("output_text")
                                or sm.get("value")
                            )
                            if isinstance(sv, dict):
                                sv = sv.get("value")
                            if isinstance(sv, str):
                                item_info["summary_preview"] = (
                                    sv[:50] + "..." if len(sv) > 50 else sv
                                )
                                ...
                                if isinstance(c, dict):
                                    ...
                                    if "output_text" in c:
                                        ...
                                        if isinstance(ot, str):
                                            c_info["output_text_preview"] = (
                                                ot[:50] + "..." if len(ot) > 50 else ot
                                            )
...
class OpenAIAdapter:
    def __init__(
        self, client: Optional[AsyncOpenAI] = None, api_key: Optional[str] = None
    ):
    ...
    async def generate(
        ...
        if not text:
            # Enhanced debugging
            if os.getenv("ADAPTER_DEBUG", "0").strip().lower() in (
                "1",
                "true",
                "yes",
                "on",
                ...
                if peek:
                    ...
                    log.warning(
                        "adapter.peek DETAILED structure: %s",
            ...
            log.warning(
                ...
                (
                    type(getattr(resp, "output", None)).__name__
                    if getattr(resp, "output", None) is not None
                    else "None"
                ),
                (
                    type(getattr(resp, "data", None)).__name__
