...
def _env_candidates(model: str, kind: str) -> list[str]:
    ...
    """
    env_safe = re.sub(r"[^A-Za-z0-9]+", "_", model).upper()  # e.g., gpt-5-mini -> GPT_5_MINI
    return [
...
def get_prices(model: str) -> tuple[float, float]:
    ...
    if p_in is None or p_out is None:
        log.warning("Missing price for model %s (in=%r, out=%r). Costs will compute as 0.0.", model, p_in, p_out)
    return (p_in or 0.0, p_out or 0.0)

def cost_usd(model: str, prompt_tokens: int, completion_tokens: int) -> float:
    p_in, p_out = get_prices(model)
    return round((prompt_tokens / 1_000_000.0) * p_in + (completion_tokens / 1_000_000.0) * p_out, 6)

def has_prices(model: str) -> bool:
    return (_price_for(model, "PROMPT") is not None) and (_price_for(model, "COMPLETION") is not None)
